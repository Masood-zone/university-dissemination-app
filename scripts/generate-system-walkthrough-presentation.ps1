...
$slides = @(
  ...
  @{title='The problem SIDS solves'; body=@('University information often moves through fragmented, informal channels.','SIDS brings announcements, admissions, teaching communication, academic structure and finance visibility into one platform.','The goal: make communication targeted, timely, traceable and role-aware.')},
  ...
  @{title='Role-based experience'; body=@('Administrator — university-wide configuration, applications, finance, announcements and access control.','Department Administrator — people, programmes, courses and announcements within one department.','Lecturer — assigned courses, schedules and course-context messaging.','Student — dashboard, announcements, courses, finance, calendar, messaging and profile.')},
  ...
  @{title='Core academic data model'; body=@('Department contains users, programmes, courses and course offerings.','Academic Session and Semester organize when courses are available.','A Course Offering joins a course to a session, semester, department, lecturer assignment and enrolled students.','Students accumulate applications, messages, notifications, fees and payment transactions.')},
  @{title='Academic setup workflow'; body=@('Administrator creates departments, sessions and semesters.','Programmes, courses and fee configurations are defined.','Department Administrator creates active offerings and assigns lecturers.','Lecturers publish timetable details.','This setup controls who sees a course and who can communicate.')},
  ...
  @{title='From application to active student'; body=@('A new applicant receives a Student account, Student Profile and secure temporary credential.','The application enters SUBMITTED status; a fee obligation is created when programme fees exist.','Administrator reviews the application and approves or rejects it.','Approval automatically enrolls the student in matching active offerings for the current semester.')},
  ...
  @{title='Notifications and reliable delivery'; body=@('The notification center stores read/unread history for announcement, message, system, academic and payment events.','Examples: enrollment submission, application decision, password reset and published announcement.','Channel status metadata makes delivery observable.','The database notification remains available even if an external email or SMS provider is unavailable.')},
  @{title='Student academic and finance experience'; body=@('The student dashboard brings current information and action items together.','Students see relevant course offerings, academic-calendar events and finance information.','Programme fee configuration creates per-student obligations.','Fee status: Pending, Paid, Overdue or Cancelled. Transactions also track success, failure, reversal, refund and cancellation.')},

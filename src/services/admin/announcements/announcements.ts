...
export type AdminAnnouncementsListParams = {
  q?: string;
  status?: string;
  departmentId?: string;
  page?: number;
...
async function getAnnouncementDetail(
  ...
  try {
    const res = await api.get<ApiResponse<AdminAnnouncementDetail>>(
      `/administrator/announcements/${id}`,
    );
...
async function updateAnnouncement(args: {
  ...
  try {
    const res = await api.patch<ApiResponse<{ ok: true }>>(
      `/administrator/announcements/${args.id}`,
      args.input,
    );
...
export function useAdminAnnouncementDetail(id: string | null) {
  return useQuery({
    queryKey: id
      ? announcementKeys.detail(id)
      : ["admin", "announcements", "detail", null],
    queryFn: () => {
